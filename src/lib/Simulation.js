...
export class BehaviourTree {
  ...
  buildInitialTree() {
    ...
    const turretSubset = new BTNode('turret_logic', 'TURRET ASSEMBLY', 'sequence', [
    ...
    const siegeSubset = new BTNode('siege_logic', 'BASE DEMOLITION', 'sequence', [
  ...
  evaluateAction(node, params) {
    if (node.id === 'reloading') return params.ammo < 5 ? NodeStatus.SUCCESS : NodeStatus.FAILURE;
    if (node.id === 'scouting') return params.distEnemy > 80 ? NodeStatus.SUCCESS : NodeStatus.FAILURE;
    if (node.id === 'move_depot' || node.id === 'move_medkit' || node.id === 'infiltrate') return NodeStatus.RUNNING;
    return NodeStatus.SUCCESS;
  }
...
export class UtilityEngine {
  calculateScores(params) {
    ...
    // HEALING
    const uHealing = Math.pow(1 - health/100, 2);
    // DEFENSE
