<script>
  ...
  const entries = [
    { id: 'root', title: 'Glossary: Cognitive Decision Architectures', body: 'Modern Game AI must choose between complex planning and instant, reactive scores. The decision architecture defines how an agent transitions from sensing to acting.' },
    { id: 'goap', title: 'GOAP: Regressive Search', body: 'Goal-Oriented Action Planning uses A* to search from a goal back to its preconditions. For example, "Target Dead" -> Precondition "Has Ammo" -> Action "Collect Ammo". It allows for emergent problem solving.' },
    { id: 'utility', title: 'Utility: Scoring & Weights', body: 'Fuzzy logic scoring. Each action (Attack, Heal, Flee) calculates a 0-1 value based on multiple environmental inputs. Weights allow designers to prioritize survival over combat.' },
    { id: 'fuzzy', title: 'Fuzzy Logic Curves', body: 'Unlike binary "if-then" rules, fuzzy curves provide a gradient. Health <= 20% might have a utility of 1, but 21% is not 0; it might be 0.9. This creates smooth, human-like transitions.' }
  ];
